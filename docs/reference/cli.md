...
A non-empty flag wins over the environment variable. Missing directories
are created. A missing config file is created with `display_unit = "kg"`.
An invalid `display_unit` is a startup error. Invalid `[colors]` values
are not.

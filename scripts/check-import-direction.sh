#!/bin/bash
#
# Fail if domain or config packages import upward into the TUI, or
# across the declared direction. cmd/hdtools and internal/tui may
# import downward.
...
# Unquoted ${imports} is intentional: go list emits space-separated
# import paths, and we match them one per line.
check() {
  local pkg="$1"
  shift
  local imports
  ...
  for forbidden in "$@"; do
    ...
    if printf '%s\n' ${imports} | grep -qx "${mod}/${forbidden}"; then
      echo "FAIL: ${pkg} imports ${mod}/${forbidden}" >&2
      fail=1
    fi
  done
}
